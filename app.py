...
class Database:
    ...
    def create_post(self, user_id: int, content: str) -> int:
        with self.driver.session() as session:
            result = session.run(
                ...
                CREATE (u)-[:POSTED]->(p:Post {id: randomId(), content: $content, timestamp: datetime()})
                RETURN p.id AS id
                ''',
    ...
    def get_feed(self, user_id: int) -> List[dict]:
        with self.driver.session() as session:
            result = session.run(
                '''
                MATCH (u:User {id: $user_id})-[:FOLLOWS]->(followed:User)-[:POSTED]->(p:Post)
                RETURN p.id AS id, p.content AS content, p.timestamp AS timestamp,
    ...
    def get_following(self, user_id: int) -> List[dict]:
        with self.driver.session() as session:
            result = session.run(
                '''
                MATCH (u:User {id: $user_id})-[:FOLLOWS]->(target:User)
                RETURN target.id AS id, target.username AS username, target.name AS name
                ''',
                user_id=user_id
            )
            return [{'id': row['id'], 'username': row['username'], 'name': row['name']} for row in result]
